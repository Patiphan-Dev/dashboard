...
if st.sidebar.checkbox("🔍 Debug"):
    ...
    for key in st.session_state:
        if key.endswith("_data"):
            st.sidebar.write(f"{key}: {type(st.session_state[key])}, rows={len(st.session_state[key]) if isinstance(st.session_state[key], pd.DataFrame) else 'N/A'}")
    for key in st.session_state:
        if key.endswith("_file"):
            st.sidebar.write(f"{key}: {st.session_state[key]}")
    st.sidebar.write("All session state:", dict(st.session_state))
    st.sidebar.write("Supabase URL:", SUPABASE_URL)
    st.sidebar.write("Supabase Bucket:", SUPABASE_BUCKET)
    # Do not print the key for security reasons
